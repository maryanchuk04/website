...
const Employers = () => {
	...
	const refresh = async () => {
		...
		setEmployers(null);
		const { data } = await axios.get('http://college-backend.somee.com/employerspage');
		setEmployers(data);
	};
	...
	const handleSubmit = (e) => {
		...
		axios({
			...
			.then((result) => {
				axios
					.post(
						`http://college-backend.somee.com/employerspage/addemployer/${activeEmployers.id}`,
						{
							...employer,
							image: result.data,
						}
	...
	return !employers ? (
		...
		<div className='employers_wrapper'>
			<SubSidebar
				elements={employers}
				handleClick={chooseMenu}
				activeItem={activeEmployers?.name || ''}
			/>
			<div className='employers_container'>
				{activeEmployers && (
					<div>
						<div className=''>
							<button className='create_emp' onClick={() => setOpen(true)}>
								Додати спеціаліста
							</button>
						</div>
						<div>
							{activeEmployers.employers.map((item) => (
								<div className='emp_item'>
									...
									<button
										onClick={() => deleteEmployer(activeEmployers.id, item.id)}
									>
					...
					<form className='emp_create_model' onSubmit={handleSubmit}>
						...
						<div className='fields'>
							<input
								...
								required
								onChange={(i) =>
									setEmployer({ ...employer, full_name: i.target.value })
								}
							...
							<input
								...
								required
								onChange={(i) =>
									setEmployer({ ...employer, posada: i.target.value })
								}
							...
							<input
								...
								onChange={(i) =>
									setEmployer({ ...employer, number: i.target.value })
								}
							/>
							<input
								type='text'
