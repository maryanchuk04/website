...
const SliderAdmin = () => {
	...
	const refresh = async () => {
		const { data } = await axios.get('http://college-backend.somee.com/slider/all');
		setSlider(data);
	};
	...
	const deleteClick = (i, id) => {
		axios
			.delete(`http://college-backend.somee.com/slider/delete/${id}`)
			.then(async () => {
				...
				{slider.map((s, index) => (
					<div className='slider_item'>
						...
						<div className='knopku'>
							...
							/>
							<button onClick={(i) => saveSliderClick(i, s.id, number)}>
								Зберегти
							</button>
