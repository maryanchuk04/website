...
const CreateNews = ({ close }) => {
	...
	const handleSubmit = (e) => {
		...
		axios.post("https://bsite.net/IvanovIvan/news/add", {
			...news, text: editorRef.current.value
		}).then((result) => {
	...
	return (
		<form className='create_news_container' onSubmit={handleSubmit}>
			...
			<div className="create_news">
				<input type="text" placeholder="Заголовок новини" required onChange={({ target }) => setNews({ ...news, title: target.value })} />
				<input type="text" placeholder='Короткий опис новини' required onChange={({ target }) => setNews({ ...news, short_text: target.value })} />
				<input type="date" placeholder='Дата' required onChange={({ target }) => setNews({ ...news, date: target.value })} />
